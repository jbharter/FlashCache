...
public class SortedCache<K, V> extends BaseCache<K, V> {
    ...
    public void put(Collection<? extends K> c) {
        c.forEach(this::get);
    }
    ...
    public void setMapper(Function<? super K, ? extends V> map) {
        this.mapper = map;
    }

    public V get(K key) {
        return internalCache.getOrDefault(key, this.mapper != null ? put(key) : null);
    }
